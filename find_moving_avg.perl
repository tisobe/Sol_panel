...
#											#
#		example 1: perl find_moving_avg.perl input_data 10 10 4 out_data	#
#				(if you want to drop outlayers)				#
#		example 2: perl find_moving_avg.perl input_data 10 10 4 out_data -nodrop#
...
my ($file, $arange, $step, $nterm, @input, $incnt, $i, $avg, $std, $max, $min, $tot);
...
$step   = $ARGV[2];		#---- step size
$nterms = $ARGV[3];		#---- degree of polynomial fitting
$outfile= $ARGV[4];		#---- output file name
$nodrop = $ARGV[5];		#---- if this one is entired, all data will be used
...
if($file =~ /-h/i || $file eq ''){
	print 'USAGE:',"\n";
	print '        perl find_moving_avg.perl <file name> <a period> <step> <degree> <out file> ',"\n";
...
while($end < $last){
	...
	for($i = 0; $i < $dcnt; $i++){
		...
		$j = $i;
		while($time[$j] >= $start && $time[$j] < $end){
			$sum  += $mdata[$j];
			$sum2 += $mdata[$j] * $mdata[$j];
			if($mdata[$j] > $max){
				$max = $mdata[$j];
			}
			if($mdata[$j] < $min){
